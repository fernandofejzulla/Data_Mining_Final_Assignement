...
def create_user_movie_matrix(data):
    ...
    # create sparse binary matrix (1=user rated movie, 0=otherwise)
    rows = [user_to_idx[user] for user in data[:, 0]]
    cols = [movie_to_idx[movie] for movie in data[:, 1]]

    # binary matrix (1=rated, 0=otherwise) (ratings don't matter)
...
def generate_minhash_signatures(user_movie_matrix, signature_length, random_seed):
    ...
    # for each user (row), update signatures
...
def verify_similar_pairs(candidate_pairs, user_movie_matrix, user_to_idx, threshold=0.5, max_seconds=None):
    ...
    for i, (u1_idx, u2_idx) in enumerate(candidate_pairs):
        # time limit check
        if max_seconds is not None:
            elapsed = time.time() - start_time
            if elapsed > max_seconds:
                print(f"Time limit of 30 minutes reached after verifying {i} candidate pairs")
                break
...
def plot_similarities(similarities, threshold=0.5):
    # plot sorted Jaccard similarities)
    if not similarities:
